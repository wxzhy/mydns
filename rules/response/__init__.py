...
class ResponseDebugHook(ResponseHook):
    ...
    async def before_response(self, context: QueryContext) -> None:
        logger.debug(
            "最终结果 source=%s client=%s:%s txid=%s qtype=%s domain=%s winner=%s attempts=%s rtt=%s selected_ips=%s selected_ip_rtt=%s result=%s",
            ...
            ", ".join(context.selected_ips) if context.selected_ips else "-",
            (
                f"{context.selected_ip_rtt_ms:.2f}ms"
                if context.selected_ip_rtt_ms is not None
                else "-"
            ),
            summarize_dns_result(context.rcode, context.answer or []),
        )
